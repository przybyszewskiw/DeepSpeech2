...
Following description of the model architecture is pretty basic. For more detailed one please refer to \cite{DS2}.
...
normalized audio clips, so $x^{(i)}_{t,p}$ denotes the power of the $p$’th frequency bin in the audio frame at time $t$. $y^{(i)}$ is a transcription of the utterance $x^{(i)}$.\\\\
...
\begin{itemize}
  \item $\mathbb{P}_{b}(y, t)$ -- the probability of the given transcrition $y$ at time-step $t$ if the encoding ends with \textit{blank},
  \item $\mathbb{P}_{nb}(y, t)$ -- the probability of the given transcrition $y$ at time-step $t$ if the encoding doesn't end with \textit{blank},
  \item $\mathbb{P}_{lm}(y)$ -- the probability of the given transcrition $y$, according to the language model.\\
\end{itemize}
