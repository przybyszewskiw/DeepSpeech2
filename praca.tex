...
\begin{document}
	...
	To sum up, we present PyTorch implementation of Deep Speech 2 model for ASR. We also provide already trained models as well as the results of our experiments justifying using specific architecture solutions.
	...
	We also managed to speed-up training using potential of GPUs -- we trained our model in mixed precision and used multiple GPUs for it. We also showed how exactly these optimizations speed-up training process.
	...
	NVIDIA plans to publish our network as a deep speech learning example which shows how multi-GPU and mixed precision training can speed-up computations. Moreover, trained network is going to be published on NVIDIA GPU CLOUD for educational and research purposes. Corporation expressed a desire to continue project development in order to publish it on their official GitHub and on torch.hub.
